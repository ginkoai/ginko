...
export class BestPracticesManager {
    ...
    static getDefaultPractices(filters) {
        ...
        if (filters) {
            if (filters.categories?.length) {
                practices = practices.filter(p => filters.categories.includes(p.category));
            }
            if (filters.tags?.length) {
                practices = practices.filter(p => p.tags.some(tag => filters.tags.includes(tag)));
            }
            if (filters.priority?.length) {
                practices = practices.filter(p => filters.priority.includes(p.priority));
            }
        }
        return practices;
    }
     ...
     */
    static formatPracticesForContext(practices) {
     ...
     */
    static mergeWithDefaults(customPractices = [], disabledIds = []) {
        // Start with defaults, excluding disabled ones
        const enabledDefaults = this.defaultPractices.filter(p => !disabledIds.includes(p.id));
