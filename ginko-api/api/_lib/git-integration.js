...
export class GitIntegration extends EventEmitter {
     ...
     */
    async getRecentCommits(since = '1 day') {
        if (!this.isGitRepo)
            return [];
        try {
            const commits = execSync(`git log --since="${since}" --pretty=format:"%H|%an|%ae|%s|%ad" --date=iso`, { cwd: this.projectPath, encoding: 'utf-8' });
            if (!commits)
                return [];
            const events = [];
            for (const commitLine of commits.split('\n')) {
                ...
                const filesChanged = execSync(`git diff-tree --no-commit-id --name-only -r ${hash}`, { cwd: this.projectPath, encoding: 'utf-8' }).split('\n').filter(Boolean);
                events.push({
                    type: 'commit',
                    author,
                    message,
     ...
     */
    static parseGitHubWebhook(payload) {
        try {
     ...
     */
    static parseGitLabWebhook(payload) {
        try {
            if (!payload.commits || !payload.project)
                return null;
            const latestCommit = payload.commits[payload.commits.length - 1];
            if (!latestCommit)
                return null;
     ...
     */
    static getContextUpdateScope(changedFiles) {
        ...
        const needsFullScan = changedFiles.some(file => structuralFiles.some(pattern => file.includes(pattern)) ||
