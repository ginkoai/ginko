...
import { UsageEventType } from './usage-tracker.js';
export class BillingError extends Error {
    code;
    stripeError;
    constructor(message, code = 'BILLING_ERROR', stripeError) {
        super(message);
        this.code = code;
...
export class BillingManager {
     ...
     */
    async createCustomer(organizationId, billingEmail, name, address) {
        try {
      ...
      `, [customer.id, billingEmail, organizationId]);
            ...
            if (error instanceof Stripe.errors.StripeError) {
                throw new BillingError(`Failed to create customer: ${error.message}`, 'CUSTOMER_CREATION_FAILED', error);
            }
            throw error;
        }
    }
     ...
     */
    async createSubscription(organizationId, planTier, interval = 'month', paymentMethodId) {
        try {
            // Get customer
      ...
      `, [
            ...
            if (error instanceof Stripe.errors.StripeError) {
                throw new BillingError(`Failed to create subscription: ${error.message}`, 'SUBSCRIPTION_CREATION_FAILED', error);
            }
            throw error;
        }
    }
     ...
     */
    async updateSubscription(organizationId, newPlanTier, interval = 'month') {
        try {
      ...
      `, [newPlanTier, organizationId]);
            ...
            if (error instanceof Stripe.errors.StripeError) {
                throw new BillingError(`Failed to update subscription: ${error.message}`, 'SUBSCRIPTION_UPDATE_FAILED', error);
            }
            throw error;
        }
    }
    ...
    async cancelSubscription(organizationId, cancelAtPeriodEnd = true) {
      ...
      `, [organizationId]);
        ...
        `, [organizationId]);
            ...
            if (error instanceof Stripe.errors.StripeError) {
                throw new BillingError(`Failed to cancel subscription: ${error.message}`, 'SUBSCRIPTION_CANCELLATION_FAILED', error);
            }
            throw error;
        }
    }
    ...
    async handleWebhook(rawBody, signature) {
        ...
        try {
            ...
            throw new BillingError(`Webhook signature verification failed: ${error instanceof Error ? error.message : String(error)}`, 'WEBHOOK_SIGNATURE_INVALID');
        }
     ...
     */
    formatSubscription(subscription, organizationId) {
        return {
            id: subscription.id,
     ...
     */
    mapStripeStatusToPlanStatus(stripeStatus) {
        switch (stripeStatus) {
            case 'active':
                return 'active';
            case 'trialing':
     ...
     */
    async createProductAndPrices(planTier, name, description) {
        if (planTier === 'free')
            return; // Free plan doesn't need Stripe products
        // Create product
