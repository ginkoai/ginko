...
export class DatabaseManager {
  ...
  private static readonly DEFAULT_TEAM_UUID = '00000000-0000-0000-0000-000000000001';
   ...
   */
  private normalizeToUUID(id: string, type: 'team' | 'project'): string {
  ...
  async getTeamBySlug(organizationSlug: string, teamSlug: string): Promise<Team | null> {
  ...
  async createTeam(organizationId: string, name: string, slug: string, settings: Record<string, any> = {}): Promise<Team> {
    const result = await this.query<Team>(`
  ...
  async getProjectBySlug(teamId: string, projectSlug: string): Promise<Project | null> {
  ...
  async getProjectContext(projectId: string, contextType: string, contextKey: string): Promise<ProjectContext | null> {
    const result = await this.query<ProjectContext>(`
  ...
  async saveGitEvent(projectId: string, gitEvent: GitEvent, webhookPayload?: any): Promise<void> {
    await this.query(`
  ...
  async getRecentGitEvents(projectId: string, days: number = 7): Promise<GitEvent[]> {
    const result = await this.query(`
    ...
    return result.rows.map(row => ({
      ...
      message: row.message,
      files: JSON.parse(row.files_changed).map((f: any) => f.path),
  ...
  async getTeamActivity(teamId: string, projectId: string, hours: number = 24): Promise<TeamActivity[]> {
    const result = await this.query(`
      SELECT 
        id,
        team_id as "teamId",
  ...
  async getTeamBestPractices(teamId: string): Promise<BestPractice[]> {
  ...
  async saveTeamBestPractice(teamId: string, practice: BestPractice, isCustom: boolean = false): Promise<void> {
    await this.query(`
  ...
  async initializeTeamBestPractices(teamId: string, defaultPractices: BestPractice[]): Promise<void> {
  ...
  async disableTeamBestPractice(teamId: string, practiceId: string): Promise<void> {
    await this.query(`
  ...
  async enableTeamBestPractice(teamId: string, practiceId: string): Promise<void> {
    await this.query(`
      UPDATE team_best_practices 
  ...
  async saveSession(sessionContext: SessionContext): Promise<void> {
    ...
    const content = {
      ...
      handoffContent: (sessionContext as any).handoffContent
    };
  ...
  async getUserSessions(userId: string, teamId: string, limit: number = 10): Promise<Array<{
    id: string;
    projectId: string;
    currentTask: string;
    createdAt: Date;
  ...
  async createSessionSnapshot(sessionId: string, contextData: any, snapshotType: string, createdBy?: string): Promise<void> {
  ...
  async getSessionHandoffMetrics(projectId: string, days: number = 30): Promise<{
  ...
  async getHealthCheck(): Promise<{ status: string; timestamp: Date; stats: any }> {
  ...
  async getHandoffAssessments(sessionId?: string, teamId?: string, limit: number = 50): Promise<any[]> {
    try {
      let query = `
