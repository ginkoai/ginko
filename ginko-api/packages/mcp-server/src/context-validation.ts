...
export interface StalenessResult {
  ...
  score: number; // 0.0 (fresh) to 1.0 (completely stale)
  reasons: string[];
...
export class ContextValidator {
   ...
   */
  async generateContextMetadata(projectPath: string = process.cwd()): Promise<ContextMetadata> {
    try {
   ...
   */
  async validateContextStaleness(
    ...
    try {
      ...
      const architecturalChanges = await this.detectArchitecturalChanges(
        metadata.architecturalFingerprint, 
        projectPath
      );
      ...
      const reasons = this.generateStalenessReasons(
   ...
   */
  private async generateArchitecturalFingerprint(projectPath: string): Promise<string> {
   ...
   */
  private async detectArchitecturalChanges(
   ...
   */
  private getCurrentCommitHash(projectPath: string): string {
    try {
   ...
   */
  private getCommitsBehind(oldCommit: string, projectPath: string): number {
    if (oldCommit === 'unknown') return 0;
   ...
   */
  private calculateTimeScore(hours: number): number {
    if (hours <= this.STALENESS_THRESHOLDS.MAX_HOURS_FRESH) return 0.0;
    if (hours <= this.STALENESS_THRESHOLDS.MAX_HOURS_MINOR) return 0.2;
    if (hours <= this.STALENESS_THRESHOLDS.MAX_HOURS_MODERATE) return 0.5;
   ...
   */
  private calculateCommitScore(commits: number): number {
    if (commits <= 0) return 0.0;
    if (commits <= this.STALENESS_THRESHOLDS.MAX_COMMITS_FRESH) return 0.1;
    if (commits <= this.STALENESS_THRESHOLDS.MAX_COMMITS_MINOR) return 0.3;
    if (commits <= this.STALENESS_THRESHOLDS.MAX_COMMITS_MODERATE) return 0.6;
  ...
  private categorizeStaleness(
    ...
    hours: number, 
    commits: number
  ): 'fresh' | 'minor' | 'moderate' | 'critical' {
    if (score >= 0.8 || commits > 25) return 'critical';
    if (score >= 0.5 || hours > 24) return 'moderate';
    if (score >= 0.2 || commits > 3) return 'minor';
    return 'fresh';
  }
