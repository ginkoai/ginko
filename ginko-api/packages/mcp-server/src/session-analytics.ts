...
export interface SessionAnalyticsData {
  ...
  activeCodingTime: number; // seconds
  ...
  aiQueries: number;
  aiResponseTimeAvg: number; // milliseconds
...
export class SessionAnalytics {
   ...
   */
  async analyzeSession(sessionContext: SessionContext): Promise<SessionAnalyticsData> {
   ...
   */
  async storeSessionAnalytics(data: SessionAnalyticsData): Promise<void> {
    try {
      await this.db.query(`
   ...
   */
  async getFileHotspots(projectId: string, days: number = 7): Promise<FileHotspot[]> {
    try {
      const result = await this.db.query(`
        SELECT 
          file_path,
      ...
      return result.rows.map((row: any) => ({
        ...
        riskLevel: row.risk_level as 'low_risk' | 'moderate_risk' | 'high_risk'
      }));
    } catch (error) {
  ...
  ): Promise<TeamAnalyticsData | null> {
    try {
      const result = await this.db.query(`
  ...
  private calculateProductivityMetrics(sessionContext: SessionContext) {
  ...
  private calculateCollaborationMetrics(sessionContext: SessionContext) {
    return {
  ...
  private calculateFocusMetrics(sessionContext: SessionContext) {
    const completedTasks = sessionContext.openTasks.filter(task => 
      (task as any).status === 'complete'
    ).length;
   ...
   */
  async processSessionCompletion(sessionContext: SessionContext): Promise<void> {
    ...
    try {
      ...
      await this.recordSessionEvent(
        ...
        'session_complete',
        {
          ...
          tasksCompleted: sessionContext.openTasks.filter(t => (t as any).status === 'complete').length,
          filesModified: sessionContext.recentFiles.length
        },
