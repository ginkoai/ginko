...
export interface AuthenticatedUser {
  ...
  teamId?: string;         // Collaboration unit (optional, can be multiple)
  planTier: PlanTier;      // free | pro | enterprise
  planStatus: PlanStatus;  // active | past_due | canceled | trialing
...
export class AuthManager {
  ...
  private readonly KEY_PREFIX = 'wmcp'; // Ginko MCP prefix
  private readonly LEGACY_PREFIX = 'cmcp'; // Legacy ContextMCP prefix for backward compatibility
  private readonly KEY_LENGTH = 32; // 32 bytes = 256 bits
  private readonly HASH_ROUNDS = 12; // Bcrypt rounds
   ...
   */
  async generateApiKey(options: ApiKeyGenerationOptions): Promise<ApiKey> {
  ...
  async authenticateApiKey(apiKey: string): Promise<AuthenticatedUser> {
    ...
    if (environment !== expectedEnv) {
      throw new AuthenticationError(
        `API key environment mismatch: expected ${expectedEnv}, got ${environment}`,
    ...
    const authenticatedUser: AuthenticatedUser = {
      ...
      planTier: (row.subscription_tier || 'free') as PlanTier,
      planStatus: 'active' as PlanStatus, // MVP: all OAuth users are active
      role: 'owner' as UserRole, // MVP: simplified role model
   ...
   */
  async rotateApiKey(userId: string, options?: Omit<ApiKeyGenerationOptions, 'userId'>): Promise<ApiKey> {
   ...
   */
  async checkPermission(user: AuthenticatedUser, permission: string): Promise<boolean> {
