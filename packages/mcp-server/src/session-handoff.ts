...
import ContextValidator, { ContextMetadata } from './context-validation.js';
...
export interface EmbeddedContext {
  ...
  contextYouNeed: {
    problem?: string;           // Exact error message
    reproduce?: string;          // Exact command to reproduce
    lastWorkingState?: string;   // What worked before it broke
    hypothesis?: string;         // Current theory
  };
  progressSnapshot: {
    completed: Array<{ task: string; result: string }>;
    inProgress: Array<{ task: string; status: string }>;
    blocked?: Array<{ task: string; blocker: string }>;
    timeEstimate: string;
  };
  ...
  watchouts?: string[];  // 2-3 critical warnings only
}
...
Include specific feedback on what could be improved.`;
   ...
   */
  async generateHandoffCreationPrompt(
    userId: string,
    teamId: string,
    projectId: string,
    currentTask: string
  ...
  async storeHandoffContent(
    ...
    await this.storeSessionContext(sessionContext as any);
   ...
   */
  async loadHandoffForResumption(sessionId: string): Promise<string | null> {
    ...
    return (session as any).handoffContent || null;
  }

  async loadMostRecentHandoff(userId: string, teamId: string, projectId: string): Promise<string | null> {
    ...
    try {
      ...
      if (projectSessions.length > 0) {
        ...
        if (sessionContext && (sessionContext as any).handoffContent) {
          return (sessionContext as any).handoffContent;
        }
      }
  ...
  async captureSession(
    ...
    teamId: string,
    projectId: string,
    options: Partial<SessionHandoffOptions> = {}
    ...
    const rapportContext = options.rapportContext || await this.generateRapportContext(
      currentMode,
      nextMode,
      workingContext.currentTask,
      developmentState,
      userId
    );
    
    // Generate embedded context if not provided
    const embeddedContext = options.embeddedContext || await this.generateEmbeddedContext(
      nextMode,
      developmentState,
      problemSolvingContext,
      await this.captureRecentCommands()
    );
   ...
   */
  async listUserSessions(userId: string, teamId: string): Promise<Array<{
    id: string;
    projectId: string;
    currentTask: string;
    createdAt: Date;
  ...
  private async analyzeWorkingContext(): Promise<{
    currentTask: string;
    focusAreas: string[];
  }> {
  ...
  private async extractConversationContext(options: Partial<SessionHandoffOptions>): Promise<{
    summary: string;
    decisions: Array<{
      decision: string;
      rationale: string;
  ...
  private async captureDevelopmentState(): Promise<{
    recentFiles: Array<{
      path: string;
      lastAccessed: Date;
      purpose: string;
  ...
  private async extractProblemSolvingContext(): Promise<{
    challenges: Array<{
      problem: string;
      attempts: string[];
      currentApproach: string;
  ...
  private async captureRecentCommands(): Promise<Array<{
    command: string;
    output: string;
    timestamp: Date;
    success: boolean;
  ...
  private async storeSessionContext(context: SessionContext): Promise<void> {
    // Use the existing database method for session storage
    await this.db.saveSession(context);
  }

  private async loadSessionContext(sessionId: string): Promise<SessionContext | null> {
    // Use the existing database method for session loading
    return await this.db.loadSession(sessionId);
  }

  private async generateResumptionPrompt(context: SessionContext): Promise<string> {
    const sections = [];
      ...
      context.openTasks.forEach((task, i) => {
        if (typeof task === 'string') {
          ...
          if ((task as any).status) {
            sections.push(`**Status**: ${(task as any).status}`);
          }
          if (task.nextSteps) {
    ...
    if (context.recentFiles.length > 0) {
      ...
      context.recentFiles.forEach(file => {
        if (typeof file === 'string') {
          ...
          sections.push(`- **${file.path}**: ${file.purpose || (file as any).description || (file as any).type}`);
          if ((file as any).modifications && (file as any).modifications.length > 0) {
            sections.push(`  - Recent changes: ${(file as any).modifications.join(', ')}`);
          }
          if ((file as any).lastModified) {
            sections.push(`  - Last modified: ${new Date((file as any).lastModified).toLocaleString()}`);
          }
        }
      });
      sections.push(``);
  ...
  private async generateSetupCommands(context: SessionContext): Promise<string[]> {
    const commands = [];
  ...
  private async detectCurrentMode(
    developmentState: any,
    problemSolvingContext: any
  ): Promise<SessionMode> {
    ...
    if (developmentState.activeFeatures.some((f: any) => f.status === 'planning')) {
      return 'planning';
    }
    
    if (developmentState.activeFeatures.some((f: any) => f.status === 'testing')) {
      return 'shipping';
    }
  ...
  private async predictNextMode(
    ...
    const hasBlockers = problemSolvingContext.challenges.some((c: any) => c.blockers?.length > 0);
    const hasPlannedFeatures = developmentState.activeFeatures.some((f: any) => f.status === 'planning');
  ...
  private async generateModeRationale(
    currentMode: SessionMode,
    nextMode: SessionMode,
    developmentState: any
  ): Promise<string> {
  ...
  private async generateRapportContext(
    currentMode: SessionMode,
    nextMode: SessionMode,
    currentTask: string,
    developmentState: any,
    userId?: string
  ): Promise<RapportContext> {
    ...
    const hasBlockers = developmentState.openTasks.some((t: any) => 
      t.blockers?.length > 0 || t.status === 'blocked'
    );
    const completedCount = developmentState.openTasks.filter((t: any) => 
      t.progress?.includes('complete') || t.progress?.includes('done')
    ).length;
    const hasFailures = developmentState.recentCommands?.some((c: any) => !c.success);
    ...
    const contextualMood: RapportContext['contextualMood'] = {
  ...
  private async generateEmbeddedContext(
    nextMode: SessionMode,
    developmentState: any,
    problemSolvingContext: any,
    recentCommands: any[]
    ...
    const uncommittedFiles = developmentState.recentFiles
      ?.filter((f: any) => f.modifications?.length > 0)
      ?.map((f: any) => f.path)
      ?.slice(0, 5);
    
    // Determine first action based on mode and state
    const firstTask = developmentState.openTasks.find((t: any) => 
      !t.progress?.includes('complete') && !t.blockers?.length
    );
    ...
    const contextYouNeed: EmbeddedContext['contextYouNeed'] = {};
    ...
    if (problemSolvingContext.challenges[0]) {
      ...
      const lastSuccess = reversedCommands.find((c: any) => c.success);
      if (lastSuccess) {
    ...
    const completed = developmentState.openTasks
      .filter((t: any) => t.progress?.includes('complete'))
      .map((t: any) => ({
        task: t.description,
    ...
    const inProgress = developmentState.openTasks
      .filter((t: any) => !t.progress?.includes('complete') && !t.blockers?.length)
      .map((t: any) => ({
        task: t.description,
    ...
    const blocked = developmentState.openTasks
      .filter((t: any) => t.blockers?.length > 0)
      .map((t: any) => ({
        task: t.description,
  ...
  private estimateTimeRemaining(tasks: any[]): string {
