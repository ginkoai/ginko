 ...
 * @status: current
 * @updated: 2026-01-19
 * @tags: [api, sprint-status, history, epic-015, sprint-0, graph-authoritative]
 ...
 * Get status change history for a sprint (EPIC-015 Sprint 0 Task 5)
 *
 * Query Parameters:
...
export async function GET(
  ...
  console.log('[Sprint Status History API] GET /api/v1/sprint/:id/status/history called');
  ...
  try {
    ...
    try {
      ...
      const sprintCheck = await session.executeRead(async (tx) => {
        return tx.run(
          `MATCH (s:Sprint {id: $sprintId, graph_id: $graphId}) RETURN s.id as id`,
          { sprintId, graphId }
        );
      });

      if (sprintCheck.records.length === 0) {
        return NextResponse.json(
      ...
      // Get status history
      const history = await getStatusHistory(session, 'sprint', sprintId, graphId, limit);
    ...
    console.error('[Sprint Status History API] ERROR:', error);
    return NextResponse.json(
